...
- Runtime SVG assets (including `sac_background_overlay.svg`) live under `src/assets/` and are copied into `dist/` during `npm run build`, so releases always package a fresh overlay without committing binaries to git.
